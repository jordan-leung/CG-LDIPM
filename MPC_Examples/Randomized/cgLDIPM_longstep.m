function [x,output,exitFlag] = logInteriorPoint(W,c,A,b,v0,opts)
...
mu1 = 100*mu;
k1 = 1/sqrt(mu1);
[x1Hat,d1Hat,cg1] = solveNewtonStep(mu1,v,W,c,A,b,maxCGIter,CGTol,x);


% Sample point 2
mu2 = mu;
k2 = 1/sqrt(mu2);
[x2Hat,d2Hat,cg2] = solveNewtonStep(mu2,v,W,c,A,b,maxCGIter,CGTol,x);
...
if  startFlag < 1
    while norm(d,'inf') > 1
        ...
        [x,d,cg1] = solveNewtonStep(mu,v,W,c,A,b,maxCGIter,CGTol,x);

        % Store variables
        CGIters(numIter,:) = [cg1 0];
        muVec(numIter) = mu;
...
while mu > mu_f || norm(d,'inf') > 1
    ...
    mu1 = 100*mu;
    k1 = 1/sqrt(mu1);
    [x1,d1Hat,cg1] = solveNewtonStep(mu1,v,W,c,A,b,maxCGIter,CGTol,x);
    
    % Sample point 2
    mu2 = mu;
    k2 = 1/sqrt(mu2);
    [x2,d2Hat,cg2] = solveNewtonStep(mu2,v,W,c,A,b,maxCGIter,CGTol,x);
