function [x,output] = cgLDIPM_longstep_inexact(W,c,A,b,v0,opts)
...
maxIter = 150; % just for initialization
...
mu2 = mu;
k2 = 1/sqrt(mu2);
[x2,d2Hat,r2,cg2,rlim2,xFeas] = solveNewtonStep(mu2,v,W,c,A,b,const,maxCGIter,CGTol,x,xFeas);

% Sample point 2
mu1 = 100*mu2;
k1 = 1/sqrt(mu1);
[x1,d1Hat,r1,cg1,rlim1,xFeas1] = solveNewtonStep(mu1,v,W,c,A,b,const,maxCGIter,CGTol,x,xFeas);
CGIters(numIter,:) = [cg1 cg2];
CGres(numIter,:) = [rlim1 rlim2];

% Solve for muStar
[muStar,dStar,xStar] = muStarSolve(d1Hat,d2Hat,k1,k2,x1,x2,const);
...
while mu > mu_f || norm(d,'inf') > 1
    ...
    mu2 = mu;
    k2 = 1/sqrt(mu2);
    [x2,d2Hat,r2,cg2,rlim2,xFeas2] = solveNewtonStep(mu2,v,W,c,A,b,const,maxCGIter,CGTol,x,xFeas);
    CGIters(numIter,2) = cg2;
    CGres(numIter,2) = rlim2;

    % Only perform the search if d2Hat is less than 1
    if norm(d2Hat,'inf') >= 1 - epsilon || mu <= mu_f
        d = d2Hat;
        x = x2;
        xFeas = xFeas2;
        cg1 = 0;
...
%         mu1 = 1e6;
        ...
        [x1,d1Hat,r1,cg1,rlim1,xFeas1] = solveNewtonStep(mu1,v,W,c,A,b,const,maxCGIter,CGTol,x,xFeas);
        % Solve for muStar
        [mu,d,x] = muStarSolve(d1Hat,d2Hat,k1,k2,x1,x2,const);
        xFeas = x;
    end
...
if min(lambda) > -1e-4
    ...
    [x,d,r,cg] = solveNewtonStep(mu,v,W,c,A,b,const,maxCGIter,CGTol,x,x,1);
    alpha = min(1, 1/(norm(d,'inf')^2));
    v = v + alpha*d;
...
function [x,d,r,numIter,resLim,xFeas] = solveNewtonStep(mu,v,W,c,A,b,const,maxIter,tol,x0,xFeas,exactFlag)
if nargin < 12
    exactFlag = 0;
end
...
function  [muStar,d,x] = muStarSolve(d1Hat,d2Hat,k1,k2,x1,x2,const)
