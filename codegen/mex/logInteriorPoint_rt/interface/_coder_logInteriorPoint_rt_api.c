/*
 * _coder_logInteriorPoint_rt_api.c
 *
 * Code generation for function '_coder_logInteriorPoint_rt_api'
 *
 */
...
static emlrtRTEInfo k_emlrtRTEI = { 1, /* lineNo */
  1,                                   /* colNo */
  "_coder_logInteriorPoint_rt_api",    /* fName */
  ""                                   /* pName */
};
...
static real_T (*p_emlrt_marshallIn(const emlrtStack *sp, const mxArray *src,
  const emlrtMsgIdentifier *msgId))[200]
{
  static const int32_T dims[2] = { 20, 10 };
  ...
  mxArray *plhs[4])
{
  ...
  emxInit_real_T(&st, &xError_vec, 1, &k_emlrtRTEI, true);
  ...
  A = g_emlrt_marshallIn(&st, emlrtAlias(prhs_copy_idx_2), "A");
  ...
  xStar = e_emlrt_marshallIn(&st, emlrtAlias(prhs[8]), "xStar");
  xTol = k_emlrt_marshallIn(&st, emlrtAliasP(prhs[9]), "xTol");

  /* Invoke the target function */
  logInteriorPoint_rt(&st, *W, *c, *A, *bineq, mu_f, mu_0, *v0, maxIter, *xStar,
                      xTol, *x, xError_vec, &execTime, &numIter);
...
/* End of code generation (_coder_logInteriorPoint_rt_api.c) */
