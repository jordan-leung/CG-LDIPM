...
for iOuter = 4:4
    % Load data
    fullFileName = strcat("/Users/jordan/Documents/GitHub/CG-LDIPM/MM_Examples/data/",FileNames(iOuter));
    load(fullFileName); % Loads variables A, P, l, m, n, q, r, u
    H = P; % rename
    c = q;
    APrime = A; % rename these to have access later
    nPrime = n;
    mPrime = m;

    % Initialize A and b. This will be cut down to delete inf elements
    A = zeros(2*m,n);
    b = zeros(2*m,1);
    count = 0;
    for i = 1:m % Loop through u
        if u(i) < 1e12
            count = count + 1;
            A(count,:) = APrime(i,:);
            b(count) = u(i);
    ...
    % xStar = quadprog(H,c,A,b)
    % costVal = 0.5*xStar'*H*xStar + c'*xStar + r
