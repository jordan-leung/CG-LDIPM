...
while mu > mu_f
    ...
    dNorm = 10;
    while dNorm > 1
        [x,d,cgIters_i,r_i,minCond_i,minInd_i,condVec_i,sigma_i] =  solveNewtonStep(mu,v,const,maxCGIter,CGTol,x);
...
if min(lambda) > -1e-4
    ...
    [x,d,cg] = solveNewtonStep(mu,v,const,maxCGIter,CGTol,x,1);
...
function [x,d,numIter,r,minCond,minInd,condVec,sigma] = solveNewtonStep(mu,v,const,maxIter,tol,x0,exactFlag)
if nargin < 7
    exactFlag = 0;
end
...
while numIter  < maxIter && res > tol && truncCond ==  0
    ...
    if (cond1 > 0 && sigma*res < cond1) && ~exactFlag
        truncCond = 1;
    end
end
if numIter == 1
