...
numIters_cglow = zeros(numSample,1);
...
for iOuter = 1:numSample
    ...
    for j = 1:NumAverage
        if useMexFlag
            [~,xError,execTime_i,numIter] = logInteriorPoint_rt_mex(H,c,A,b,mu_f,mu_0,v0,maxIter,xStar,xTol);
        else
...
for iOuter = 1:numSample
    ...
    for j = 1:NumAverage
        if useMexFlag
            [~,xError,execTime_i,numIter] = logInteriorPoint_conjgrad_rt_mex(H,c,A,b,mu_f,mu_0,v0,maxIter,maxCGIter,0,xStar,xTol);
        else
...
% CG low CG iterations
fprintf('----------- Running CG-low ---------- \n')
lessIters = 7;
brokeFlag = 0;
for iOuter = 1:numSample
    % Get optimal solution
    ...
    for j = 1:NumAverage
        if useMexFlag
            [~,xError,execTime_i,numIter] = logInteriorPoint_conjgrad_rt_mex(H,c,A,b,mu_f,mu_0,v0,maxIter,lessIters,0,xStar,xTol);
        else
            [~,xError,execTime_i,numIter] = logInteriorPoint_conjgrad_rt(H,c,A,b,mu_f,mu_0,v0,maxIter,lessIters,0,xStar,xTol);
        end
        execTimeSum = execTimeSum + execTime_i;
        if xError > xTol
            brokeFlag = 1;
    ...
    if brokeFlag == 1
        fprintf('Error unachievable for CG Low at xTol: %0.2e \n',xTol)
        fprintf('Produced Error: %0.2e \n',xError(end))
        numSample = iOuter-1;
        break
    else
        execTime_cglow(iOuter) = execTimeSum/NumAverage;
        numIters_cglow(iOuter) = numIter;
    end
end
...
%         for j = 1:NumAverage
%             if useMexFlag
%                 [xx,numIter,xError,execTime_i] = projGradSolver_rt_mex(H,c,zeros(size(H,1),1),xmin,xmax,50000,xStar,xTol);
%             else
...
execTime_cglow = execTime_cglow(1:numSample);
execTime_cgdiag = execTime_cgdiag(1:numSample);
numIters_reg = numIters_reg(1:numSample);
numIters_cg = numIters_cg(1:numSample);
numIters_cglow = numIters_cglow(1:numSample);
numIters_low = numIters_cgdiag(1:numSample);
...
hold on; grid on; box on
h3 = loglog(xTolVec,execTime_cglow,'linewidth',2);
...
legend([h1 h2 h3 h4],'Direct','CG','CG 7 Iter','interpreter','latex','fontsize',12,'location','best')
