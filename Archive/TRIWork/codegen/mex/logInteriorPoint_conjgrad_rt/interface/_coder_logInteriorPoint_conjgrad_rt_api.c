  ...
  mxArray * const prhs[11], int32_T nlhs, const mxArray *plhs[5])
{
  ...
  CGTol = g_emlrt_marshallIn(&st, emlrtAliasP(prhs[9]), "CGTol");
  preCondFlag = g_emlrt_marshallIn(&st, emlrtAliasP(prhs[10]), "preCondFlag");

  /* Invoke the target function */
  logInteriorPoint_conjgrad_rt(SD, &st, *W, *c, *Aineq, *bineq, mu_f, mu_0, *v0,
    maxIter, maxCGIter, CGTol, preCondFlag, *x, &mu, &execTime, &numIter,
    &totalCGIters);
