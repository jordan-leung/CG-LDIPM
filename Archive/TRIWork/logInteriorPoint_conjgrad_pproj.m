...
[d1Hat,d2Hat,cg1,res1,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,zeros(m,1),preCondFlag);
...
while mu > mu_f || norm(d,'Inf') > 1
    ...
    [d1Hat,d2Hat,cg1,res1,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,d1Hat,preCondFlag);
...
function [d,dNext,numIter,resStar,applyPreCond] = solveNewtonStep(mu,muNext,v,const,d0,preCondFlag)
% W = const.W;
% invW = const.invW;
c = const.c;
ACon = const.A;
...
if ~isempty(muNext)
    coeff_i = p'*f_next/(p'*w);
    dNext = coeff_i*p; % initialize
end
...
while numIter  < maxIter && res/bNorm > tol
    ...
    if ~isempty(muNext)
        coeff_i = p'*f_next/(p'*w);
        dNext = dNext + coeff_i*p; % initialize
    end
...
% norm(DTemp*MTemp*DTemp*(d0.*MTilde) - DTemp*b)
