...
d = zeros(m,1);
while dNorm > 1e-8
    [d,CGIter_i,res] = solveNewtonStep(mu,v,const,d*0,preCondFlag,maxCGIter,CGTol,params);
    dNorm = norm(d,'inf');
...
function [d,numIter,res] = solveNewtonStep(mu,v,const,d0,preCondFlag,maxIter,tol,params)
% W = const.W;
% invW = const.invW;
c = const.c;
ACon = const.A;
...
% Keep a running calculation of d_MNorm 
d_MNorm = alpha^2*pNorm; % this is reealy d_MNorm^2
gamma = params.gamma;
eBound = min(res, sqrt(gamma*fNorm - d_MNorm));


% Iterate... 
dNorm = norm(x);
while numIter  <= maxIter && res > tol && ~(checkTruncCriteria(eBound,dNorm,const.params))
    zPrev = z;
    if applyPreCond
        z = r./MTilde;
    else
    ...
    % Update the dNorm
    dNorm = norm(x);
    eBound = min(res, sqrt(gamma*fNorm^2 - d_MNorm));
