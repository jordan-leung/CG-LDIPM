function [x,output] = logInteriorPoint_conjgrad_cheat(W,c,A,b,v0,opts)
...
h_vHat_vBar = zeros(maxIter,1);
h_vHat_v = zeros(maxIter,1);
h_vBar_v = zeros(maxIter,1);
...
gamma = 0.5; % parameter for the  limit of residual
...
while mu > mu_f
    ...
    for i = 1:N % iterates for static mu
        ...
        if centFlag
            hBar = hDiv(v_c,v_r);
            h_mu = hDiv(v_c,v);
            cond1 = qFunc(sqrt(h_mu) + sqrt(hBar)) <= 0.5;
            cond2 = qFunc(qFunc(sqrt(h_mu) + sqrt(hBar)) + sqrt(hBar)) <= qFunc(qFunc(h_mu)) + gamma*pFunc(h_mu);
            cond = cond1 && cond2;
        else
            cond = false;
        end
        ...
        muVec(iOuter) = mu;
        h_vHat_vBar(iOuter) = hBar;
        h_vHat_v(iOuter) = h_mu;
        h_vBar_v(iOuter) = hDiv(v_r,v);
...
output.h_vHat_vBar = h_vHat_vBar(1:maxIter);
output.h_vHat_v = h_vHat_v(1:maxIter);
output.h_vBar_v = h_vBar_v(1:maxIter);
