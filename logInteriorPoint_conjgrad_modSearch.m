function [x,lambda,s,v,mu,startFlag,numIter,muStar,exitFlag,execTime,CGIters,CGres,warmStartRes,CGerror] = logInteriorPoint_conjgrad_modSearch(W,c,Aineq,bineq,mu_f,mu_0,v0,maxIter,maxCGIter,CGTol,preCondFlag,printFlag)
...
beta_cg = 0.5;
epsilon_cg = max([ (1-beta_cg)/(a0 + a1/sqrt(mu)) , CGTol]);
[d1Hat,d2Hat,f2,MTilde,cg1,res1,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,maxCGIter,epsilon_cg,zeros(m,1),preCondFlag);

% Run seconds Newton system starting at warm-start d2Hat
[d2Hat,cg2,res2,resInit] = solveNewtonStep_warmStart(f2,v,const,maxCGIter,epsilon_cg,d2Hat,MTilde,appliedPreCond);
...
% Solve for muStar
cgres = max([res1 res2]);
[muStar,dStar] = muStarSolve(d0,d1,c0,c1,cgres,mu_f);
...
if startFlag < 1
    init = 1;
    while norm(d,'Inf') > 1 - (a0 + a1/sqrt(mu))*cgres || init == 1
        if init == 1
            init = 0;
        ...
        [d,~,~,~,cg,cgres,CGpcflag] = solveNewtonStep(mu,[],v,const,maxCGIter,epsilon_cg,d,preCondFlag);
        CGIters(numIter+1,:) = [cg1 - (a0 + a1/sqrt(mu))*cgres  cg];
        CGres(numIter+1,:) = [cgres cgres];
...
while mu > mu_f || dNorm > 1 || numIter >= maxIter  || imperfectFlag
    ...
    epsilon_cg = max([ (1-beta_cg)/(a0 + a1/sqrt(mu)) , CGTol]);
    [d1Hat,d2Hat,f2,MTilde,cg1,res1,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,maxCGIter,epsilon_cg,d1Hat,preCondFlag);
...
%     CGerror(numIter+1,:) = [error1 error2];
    ...
    [muStar,d] = muStarSolve(d0,d1,c0,c1,cgres,mu_f);
...
function  [muStar,d] = muStarSolve(d0,d1,c0,c1,cgres,mu_f)
% // Returns the largest k satisfying | d0 + k * d1 |_{infty} \le dinfmax
upper_bound = 1e14;
lower_bound =  0;
m = length(d0);
topConst = (abs(c0)+abs(c0-1))*cgres;
bottomConst = (abs(c1)+1)*cgres;
for i  = 1:m 
    % d0 + d1 k <= 1
    % -d0 - d1 k <=1 -> k 
    upper_bound_i = (1 - d0(i) - topConst ) / (d1(i) + bottomConst);
    lower_bound_i = (-1 - d0(i) + topConst) / (d1(i) - bottomConst);
