function [x,mu,execTime,numIter,totalCGIters] = logInteriorPoint_conjgrad_rt(W,c,Aineq,bineq,mu_f,mu_0,v0,maxIter,maxCGIter,CGTol,preCondFlag)
...
[d1Hat,d2Hat,f2,MTilde,CGIters,~,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,maxCGIter,CGTol,zeros(m,1),preCondFlag);
totalCGIters = totalCGIters + CGIters;

% Run seconds Newton system starting at warm-start d2Hat
[d2Hat,CGIters] = solveNewtonStep_warmStart(f2,v,const,maxCGIter,CGTol,d2Hat,MTilde,appliedPreCond);
totalCGIters = totalCGIters + CGIters;
...
if startFlag < 1
    ...
    while norm(d,'inf') > 1 || init == 1
        ...
        [d,~,~,~,CGIters] = solveNewtonStep(mu,[],v,const,maxCGIter,CGTol,d,preCondFlag);
        totalCGIters = totalCGIters + CGIters;
...
while (mu > mu_f || dNorm > 1) && numIter < maxIter
    ...
    [d1Hat,d2Hat,f2,MTilde,CGIters,~,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,maxCGIter,CGTol,d1Hat,preCondFlag);
    totalCGIters = totalCGIters + CGIters;
