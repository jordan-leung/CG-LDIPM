function [x,output] = cgLDIPM_longstep_inexact(W,c,Aineq,bineq,v0,opts)
...
mu2 = mu;
k2 = 1/sqrt(mu2);
[x2,d2Hat,r2,cg1] = solveNewtonStep(mu2,v,const,maxCGIter,CGTol,x);

% Sample point 2
mu1 = 1000*mu2;
k1 = 1/sqrt(mu1);
[x1,d1Hat,r1,cg2] = solveNewtonStep(mu1,v,const,maxCGIter,CGTol,x);
...
while mu > mu_f || norm(d,'inf') > 1
    ...
    if printFlag
        if flag == 1
            fprintf('mu = %0.2e, d = %0.4f (Constant) \n',mu,norm(d,'inf'))
        elseif flag == 2
            fprintf('mu = %0.2e, d = %0.4f (Incremented) \n',mu,norm(d,'inf'))
        end
    end
...
function [x,d,r,numIter] = solveNewtonStep(mu,v,const,maxIter,tol,x0)
W = const.W;
% invW = const.invW;
c = const.c;
A = const.A;
...
% First off, we should check if it is even less than the current value of
% mu
if mu > mu_init
    muStar = mu_init;
    d = d2Hat;
    ...
    if sigma*norm(r,2) >(1-gamma)*dNorm % then we need to do bisection
        % Inialize
        numIter_bs =  0;
        maxIter_bs = 500;
        mu_true = mu_init;
        ...
        foundFlag = 0;
        % Run bisection
        while numIter_bs < maxIter_bs && (mu_true - mu_false) > 0.01*mu_init
            % Set mu
            numIter_bs = numIter_bs + 1;
            ...
            barGrad_i = A'*sInv_i;
            sigma = (2/(minEig*mu))*norm(W*x + c -  mu*barGrad_i,2);
            if sigma*norm(r) > (1-gamma)*norm(d)
                mu_false = mu;
            else
                mu_true = mu;
                d_true = d;
