function [x,lambda,s,v,mu,startFlag,numIter,muStar,exitFlag,execTime,CGIters,CGres,preCondHist,warmStartRes] = logInteriorPoint_conjgrad_direct(W,c,Aineq,bineq,mu_f,mu_0,v0,maxIter,maxCGIter,preCondFlag,printFlag)
...
mu2ScaleFactor = 0.9;
mu2 = 1;
% mu2 = mu2ScaleFactor*mu;
k2 = 1/sqrt(mu2);

% Run first Newton system, generate a warm-start d2Hat for the 2nd
[d1Hat,d2Hat,R,alphaVec,betaVec,cg1,res1,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,zeros(m,1),preCondFlag);
MTilde = ones(m,1) + exp(v).*GDiag.*exp(v);

% Run seconds Newton system starting at warm-start d2Hat
[d2Hat,cg2,res2,resInit] = solveNewtonStep_warmStart(mu2,v,const,d2Hat,appliedPreCond);
CGIters(numIter+1,:) = [cg1 cg2];
CGres(numIter+1,:) = [res1 res2];
preCondHist(numIter+1) = appliedPreCond;
...
if startFlag < 1
    ...
    while norm(d,'Inf') > 1 || init == 1
        ...
        [d,~,~,~,~,cg,res,CGpcflag] = solveNewtonStep(mu,[],v,const,d,preCondFlag);
        CGIters(numIter+1,:) = [cg cg];
        CGres(numIter+1,:) = [res res];
        preCondHist(numIter+1) = CGpcflag;
...
while mu > mu_f || norm(d,'Inf') > 1
    ...
    f_new = ones(m,1) - 1/sqrt(mu1)*exp(v).*(A*invWTimes(sqrt(mu1)*A'*exp(v) - c,const) + bineq);
    dInit_pre = runWSRoutine(f_new,R,alphaVec,betaVec);
    if appliedPreCond
        dInit = dInit_pre./MTilde + d0Prev;
    else
        dInit = dInit_pre + d0Prev;
    end
    [d1Hat,d2Hat,R,alphaVec,betaVec,cg1,res1,appliedPreCond] = solveNewtonStep(mu1,mu2,v,const,dInit,preCondFlag);
    MTilde = ones(m,1) + exp(v).*GDiag.*exp(v);
    d0Prev = dInit;
    ...
    if isinf(mu)
        error('Need to edit this catch so that it works for fixed mu')
        mu = mu1;
        d = d1Hat;
    end
...
preCondHist = preCondHist(1:numIter);
...
function [d,dNext,R,alphaVec,betaVec,numIter,resStar,applyPreCond] = solveNewtonStep(mu,muNext,v,const,d0,preCondFlag)
% W = const.W;
% invW = const.invW;
c = const.c;
ACon = const.A;
...
if ~isempty(muNext)
    f_next = ones(m,1) - 1/sqrt(muNext)*exp(v).*(ACon*invWTimes(sqrt(muNext)*ACon'*exp(v) - c,const) + bCon);
end
...
% --------------- CONJUGATE GRADIENT ---------------
R = zeros(m,maxIter);
alphaVec = zeros(1,maxIter);
betaVec = zeros(1,maxIter);
...
while numIter  < maxIter && res > tol
    ...
    R(:,numIter+1) = r;
    alphaVec(:,numIter+1) = alpha;
    betaVec(numIter) = beta;
...
% Cut
R = R(:,1:numIter);
alphaVec = alphaVec(1:numIter);
betaVec = betaVec(1:numIter-1);

end


function [d,numIter,resStar,resInit] = solveNewtonStep_warmStart(mu,v,const,d0,applyPreCond)
% W = const.W;
% invW = const.invW;
c = const.c;
ACon = const.A;
...
% % % For troubleshooting
% DTemp = diag(1./sqrt(MTilde));
% MTemp = eye(m) + diag(exp(v))*ACon*const.invW*ACon'*diag(exp(v));

% Define the RHS vector b
b = ones(m,1) - 1/sqrt(mu)*exp(v).*(ACon*invWTimes(sqrt(mu)*ACon'*exp(v) - c,const) + bCon); 
...
function xNext = runWSRoutine(b,R,alphaVec,betaVec)
% Run the first iteration of CG and iniialize iteration variables
k = size(R,2);
r = R(:,1);
alpha = alphaVec(1);
res = norm(r,2); % initialize
